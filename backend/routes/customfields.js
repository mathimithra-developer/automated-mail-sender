...
router.post('/', async (req, res) => {
  try {
    ...
    // Check for duplicate custom field name or label in this org (case-insensitive check)
    const systemFields = [
    ...
    const isSystemDuplicate = systemFields.some(f => 
      f.label.toLowerCase() === label.trim().toLowerCase() ||
      f.name.toLowerCase() === cleanName.toLowerCase()
    );
    if (isSystemDuplicate) {
      return res.status(400).json({ error: 'A field with this name already exists. Duplicate field names are not allowed.' });
    }
