...
router.get('/:id', async (req, res) => {
  try {
    const orgId = req.session?.orgId;
    const test = await ABTest.findOne({ _id: req.params.id, organization: orgId })
      .populate('campaignA')
      .populate('campaignB')
      .lean();
...
router.post('/:id/pick-winner', async (req, res) => {
  try {
    ...
    const test = await ABTest.findOne({ _id: req.params.id, organization: orgId }).populate('campaignA campaignB');
    if (!test) return res.status(404).json({ error: 'Not found' });
...
router.delete('/:id', async (req, res) => {
  try {
    const orgId = req.session?.orgId;
    const test = await ABTest.findOne({ _id: req.params.id, organization: orgId });
    if (!test) return res.status(404).json({ error: 'A/B test not found or unauthorized' });

    // Also remove reference from campaigns
    await Campaign.updateMany({ abTestId: test._id }, { $unset: { abTestId: 1 }, isABTest: false });

    await ABTest.deleteOne({ _id: test._id });
